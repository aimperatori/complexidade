...
char *vout;
int pesquisa_binaria(int v[],int i,int f,int valor)
{
if (i>f) return 0;
if (i==f)
   {
...
int m=(i+f)/2;
if (valor==v[m]) return m;
if (valor<v[m])	return pesquisa_binaria(v,i,m-1,valor);
return pesquisa_binaria(v,m+1,f,valor);
}
...
{   int i;
    ...
    for (i=0;i<TAM;i++)
	   v[i]=w[i]=rand()%100;
    time(&t2);
    printf("\n Gerou os dados em %ld segundos \n",t2-t1);
    time(&t1);
    quick_sort(v,0,TAM-1);
    time(&t2);
    printf("\n Ordenado por quick em %ld segundos \n",t2-t1);
    for (i=0;i<TAM;i++)
	   v[i]=w[i];
    time(&t1);
    mergesort(v,0,TAM-1);
    time(&t2);
    printf("\n Ordenado por merge em %ld segundos \n",t2-t1);
    for (i=0;i<TAM;i++)
	   v[i]=w[i];
//    for (i=0;i<TAM;i++) printf("%d ",v[i]);
    ...
    time(&t2);
//    for (i=0;i<TAM;i++) printf("%d ",v[i]);
    printf("\n Ordenado por shellsort em %ld segundos \n",t2-t1);
    for (i=0;i<TAM;i++)
	   v[i]=w[i];
    time(&t1);
    bubble(v);
    time(&t2);
    printf("\n Ordenado por bubble em %ld segundos \n",t2-t1);
    for (i=0;i<TAM;i++)
	   v[i]=w[i];
    time(&t1);
    heapsort(v);
    time(&t2);
    printf("\n Ordenado por heap em %ld segundos \n",t2-t1);
    for (i=0;i<TAM;i++)
	   v[i]=w[i];
    time(&t1);
    radixsort(v,TAM);
    time(&t2);
    printf("\n Ordenado por radix em %ld segundos \n",t2-t1);
}